...
开发过程中，我们产出了以下核心规划文档，后续开发请随时参阅：
1. `游戏策划.md`：最新版本的概念设计草案（v4.0），包含世界观、核心机制深化、MVP 范围锁定。
2. `技术架构与排期.md`：定义了推荐的文件夹树结构、三大核心驱动数据结构（TimeManager, PlantResource, FarmingManager）以及 4 个月的排期甘特图。
3. `tasks_01.md` ~ `tasks_04.md`：将四个月的开发周期拆分成了 40 个具体的执行任务，涵盖了从玩具箱 Demo 到最终抛光的所有细节。
...
---
**AI 阅读提示**：
每当你被唤醒进行新的开发时，请先阅读本 `prompt.md` 以获取最新上下文，然后查看 `tasks_01.md` 中未勾选（`- [ ]`）的任务，确认当前应该处理的具体工作。所有的改动请确保遵循“数据驱动 (Resource)”与“轻量化 MVP”的原则。